...
df.coalesce(1).write.mode('overwrite').csv(f"dbfs:/FileStore/{username}/erm_out/",header=True)

# COMMAND ----------

df.coalesce(1).write.csv(f"dbfs:/FileStore/{username}/erm_out/",header=True,mode='overwrite')
...
df.coalesce(1).write.options(**options).csv(f"dbfs:/FileStore/{username}/erm_out/")

# COMMAND ----------

df.coalesce(1).write.mode('overwrite').options(**options).csv(f"dbfs:/FileStore/{username}/erm_out/")
...
spark.read.csv(f"dbfs:/FileStore/{username}/erm_out/",header=True).show()
